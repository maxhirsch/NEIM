...
def NEIM_Q(ro_sols, f_NEIM, mu, max_modes=10, train_loop_iterations=10000, theta_train_loop_iterations=20000):
    """
    ro_sols: (total number parameters, time window, 3*reduced order dimension)
    f_NEIM: (total number parameters, time window, 2*reduced order dimension)
    """
    ...
    # NEIM Step 1
    errors = np.array([np.sum(f_NEIM[i]**2) for i in range(NUM_PARAMS)])
    ...
    x_mean = np.mean(ro_sols[idx], axis=0, keepdims=True)
    x_std  = np.std(ro_sols[idx], axis=0, keepdims=True)
    x_std[x_std == 0] = 1
    x_data = torch.tensor((ro_sols[idx] - x_mean) / x_std).float()
    ...
    for i in range(NUM_PARAMS):
        ...
        for j in range(NUM_PARAMS):
            ...
            numerator += np.dot(f_NEIM[i, j], net_u_mu)#WEIGHTS[i, j] * np.dot(f_NEIM[i, j], net_u_mu)
            denominator += np.dot(net_u_mu, net_u_mu)#WEIGHTS[i, j] * np.dot(net_u_mu, net_u_mu)
        ...
        for i in range(NUM_PARAMS):
            ...
            for j in range(NUM_PARAMS):
                ...
                for k1 in range(num_nets):
                    RHS[k1] += np.dot(f_NEIM[i, j], nets_u_mu[k1])#WEIGHTS[i, j] * np.dot(f_NEIM[i, j], nets_u_mu[k1])
                    for k2 in range(num_nets):
                        LHS[k1, k2] += np.dot(nets_u_mu[k1], nets_u_mu[k2])#WEIGHTS[i, j] * np.dot(nets_u_mu[k1], nets_u_mu[k2])
